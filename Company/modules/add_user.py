    ...
    if m.from_user.id not in OWNER_ID:
        buat = [
            [
                ...
                InlineKeyboardButton("🎥 Tutorial Deploy", url="https://t.me/TutorialDeploy"),
            ]
        ]
    else:
        buat = [
...
async def _(_, callback_query):
    ...
    if user_id not in MEMBERS and user_id not in OWNER_ID:
        admin = [
            [
                InlineKeyboardButton("🧑‍💻 Admin", url="t.me/Onecuk"),
            ]
        ]
    ...
    for mod in ALL_MODULES:
        importlib.reload(importlib.import_module(f"Company.modules.{mod}"))
...
async def _(_, callback_query):
    ...
    for ub in ubot._ubot:
        list_ubot = f"🤖 **Company Ubot:** {bot.me.mention}\n\n"
